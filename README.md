...
v1.0.0 是 Clash for fnos 的首个正式大版本，核心运行架构、界面与系统网络管理能力均完成升级：

- **全 Go 后端**：Web 服务与 Root Helper 均改为静态 Go 二进制，移除 fnOS 上的 Node.js 运行时依赖。
- **全新 Vue 界面**：重构仪表盘、代理节点、订阅、规则、连接、日志和设置页面，统一 fnOS 窗口布局、亮暗主题与响应式交互。
- **可靠的 TUN 管理**：快速开关采用校验、应用、运行态确认与失败回滚流程；升级退出时等待 Mihomo 清理，启动后自动恢复托管模式的 TUN 运行态。
- **运行状态更完整**：增加持久流量累计、10 分钟实时流量历史、Mihomo 内存、出口 IP/地区与运行端口展示。
- **订阅与配置增强**：异步更新/应用、任务进度、本地 YAML 扫描导入、启动配置同步、备份及失败恢复均由 Go 服务统一处理。
- **系统集成增强**：支持安全管理代理环境变量、Core/GEO 更新、软件图标、TUN 排除网段，并完整区分 `x86_64` 与 `arm64`。

完整变更记录见 [CHANGELOG.md](CHANGELOG.md)。

> [!IMPORTANT]
> 可以从 v0.8.x 使用 fnOS 手动安装直接覆盖升级。若升级前已开启 TUN，升级期间网络会短暂中断；v1.0.0 会在旧版本退出时等待 Mihomo 清理，并在新版本启动后重新核对 TUN 实际运行状态。建议在可直接访问 NAS 的环境中执行升级。
...
- **External 模式**：检测到用户已经安装或运行 Mihomo 时，应用在启动阶段询问本次使用本机 Core 还是 Manager 托管 Core，不自动安装第二份 Mihomo。
