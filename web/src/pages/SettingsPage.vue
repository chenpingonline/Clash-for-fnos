...
const categories: Array<{ key: Section; icon: string; title: string; description: string }> = [
  ...
  { key: 'tun', icon: '◇', title: 'TUN 设置', description: 'TUN 详细参数与系统流量接管' },
...
function networkPayload() {
  ...
  return { controller: { port: Number(network.controller.port) }, mixed: { ...network.mixed, enabled: mixed }, socks: { ...network.socks, enabled: !mixed && Boolean(network.socks.enabled) }, http: { ...network.http, enabled: !mixed && Boolean(network.http.enabled) }, redir: { ...network.redir, enabled: !mixed && Boolean(network.redir.enabled) }, tproxy: { ...network.tproxy, enabled: !mixed && Boolean(network.tproxy.enabled) }, allowLan: network.allowLan, core: network.core, tun: network.tun }
}
function numericDelay(value: number | Event | undefined, fallback: number) { return typeof value === 'number' ? value : fallback }
function saveNetwork(delay: number | Event = 250) { if (!network.tun.autoRoute) network.tun.autoRedirect = false; netSave.queue(networkPayload(), numericDelay(delay, 250)) }
function dnsPayload() {
...
<template>
  <AsyncState :loading="loading" :error="error">
    <div class="settings-home"><div class="settings-accordion-list">
      <div v-for="category in categories" :key="category.key" class="settings-accordion-item" :class="{ open: open === category.key }"><button class="settings-accordion-head" type="button" :aria-expanded="open === category.key" @click="open = open === category.key ? null : category.key"><span class="settings-category-icon" :class="{ 'text-icon': category.key === 'dns' }">{{ category.icon }}</span><span class="settings-category-copy"><strong>{{ category.title }}</strong><small>{{ category.description }}</small></span><span class="settings-accordion-chevron">⌄</span></button>
