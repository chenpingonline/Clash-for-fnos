      ...
      <div v-for="category in categories" :id="category.key === 'tun' ? 'tun-settings' : undefined" :key="category.key" class="settings-accordion-item" :class="{ open: open === category.key }"><button class="settings-accordion-head" type="button" :aria-expanded="open === category.key" @click="toggleSection(category.key, $event)">
        ...
        <div v-if="open === category.key" class="settings-accordion-body">
          ...
          <div v-else-if="category.key === 'tun'" class="settings-accordion-panel tun-card" :class="{ 'tun-on': network.tun.enabled }">
            ...
            <div class="tun-option-grid"><SettingToggle v-model="network.tun.autoRoute" title="自动路由" description="自动把系统流量路由到 TUN" :disabled="tunSwitching || !tunSupported" @change="saveTunSettings('autoRoute')" /><SettingToggle v-model="network.tun.autoRedirect" title="Auto Redirect" description="Linux 自动配置 nftables/iptables TCP 重定向" :disabled="tunSwitching || !tunSupported || !network.tun.autoRoute" @change="saveTunSettings('autoRedirect')" /><SettingToggle v-model="network.tun.autoDetectInterface" title="自动检测出口网卡" description="自动选择实际的外网出口接口" :disabled="tunSwitching || !tunSupported" @change="saveTunSettings('autoDetectInterface')" /><SettingToggle v-model="network.tun.dnsHijack" title="DNS 劫持" description="劫持 UDP/TCP 53 到 Mihomo DNS 模块" :disabled="tunSwitching || !tunSupported" @change="saveTunSettings('dnsHijack')" /><SettingToggle v-model="network.tun.strictRoute" title="严格路由" description="减少流量/DNS 泄漏；复杂网络可能影响其他虚拟网卡" :disabled="tunSwitching || !tunSupported" @change="saveTunSettings('strictRoute')" /></div>
            <div class="field tun-route-exclude">
              ...
              <textarea v-model="tunRouteExcludeText" placeholder="192.168.0.0/16&#10;10.0.0.0/8&#10;fc00::/7" :disabled="tunSwitching || !tunSupported || !network.tun.autoRoute" @change="saveTunSettings('routeExcludeAddress', 80)" @input="saveTunSettings('routeExcludeAddress', 1000)" />
