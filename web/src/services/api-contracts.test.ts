...
describe('backend API compatibility', () => {
  ...
  it('opens dashboard TUN settings and saves the complete TUN configuration transactionally', () => {
    ...
    expect(dashboardControl).toContain(':initial-settings="tunSettings"')
    expect(tunSettings).toContain('if (props.initialSettings) applyResult(props.initialSettings)')
    expect(tunSettings).toContain('loading.value = !hasSettings')
